...
class PatternFinder():
    def __init__(self, tdb, per, min_ps, min_rec, maxpatternsize):
        self._tdb = tdb
        self._per = per
        self._min_ps = min_ps
    ...
    def _rp_growth(self, tree, pattern):
        ...
        """
        if len(pattern) < self.maxpatternsize or self.maxpatternsize == -1:
            for node in tree.items_ordered():
                ...
                if self._get_recurrence(curr_item, sorted(timestamps[curr_item])) and curr_item not in pattern:
        ...
        for timestamp in timestamps:
            if last_occurence is None:
                ...
                if timestamp - last_occurence <= self.per:
                    ...
                    ESR += math.floor(current_ps/self.min_ps)
                    if current_ps >= self.min_ps:
                        sub_db.append((start_ts, last_occurence))
