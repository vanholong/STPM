...
class FTPMining:
    def __init__(self, database, maxper, minSR, minPS, epsilon, minOverlap, mindist, maxdist, maxPatternSize):
    ...
    def Find2FrequentPatterns(self, candidate_patterns):
        self.Nodes['level2'] = {}

        for cadidate in candidate_patterns:
            node1_id = cadidate[0]
            if node1_id not in self.Nodes['level1']:
                continue
            ...
            if node2_id not in self.Nodes['level1']:
                continue
    ...
    def FindKFrequentPatterns(self, candidate_patterns):
        level = len(candidate_patterns[0])
        level_name = 'level{}'.format(level)
        self.Nodes[level_name] = {}

        for candidate in candidate_patterns:
            cons = True
            for id1 in range(level-1):
                for id2 in range(id1+1, level):
                    if (candidate[id1], candidate[id2]) not in self.Nodes['level2']:
                        cons = False
                        break
                if cons is False:
                    break

            if candidate[:-1] not in self.Nodes['level{}'.format(level-1)]:
                cons = False

            if cons is False:
                continue

            k_1_node = self.Nodes['level{}'.format(level-1)][candidate[:-1]]
            k_1_bitmap = set(k_1_node.get_bitmap())
            k_1_support = k_1_node.get_support()

            _1_id_event = candidate[-1]
            ...
            if ESR >= self.minSR:
                list_patterns = self.FindKPatterns(candidate[:-1], _1_id_event, level)
                list_pattern_candidates = []
